...
| **Popovers** | Now **disclosure tips**, not dialogs: no `role=dialog` / `aria-haspopup`; the tip gets `role="note"`, moves into reading order right after the block holding its trigger when opened, prefers opening **below** the trigger inside its card (scrolls into view if needed), flips above only if that fits in the card, and never overlaps another card; bottom sheet < 600px, and closes on Esc (focus returns), click outside, trigger again, or **focus leaving** trigger + tip. | Same markup as before (`data-pop` + `.popover` anywhere in the panel). Keep tip content short; links inside a tip are reachable with Tab. | No. |
...
| **Categories** | `--cat-1…5`, `--cat-other` are now **accent + ink tints** (no new hues): cat-1 = accent (the lead category), cat-2…other = ink steps starting at mid-ink and fading toward the surface in both themes, so the accent always leads. | Keep using the tokens in fixed order; always text-label. | No (colours update automatically). |
...
| **Empty / not-ready template** | New parts: `.empty__status`, `.empty__when` (mandatory), `.empty__note`. `.motif` halftone sits fully **inside** the tile's top-right corner on desktop (≥ 961px; the text column gets 136px right padding), and is dropped on smaller screens — never behind text, never peeking out. | See the template in §4 "Empty / not-ready state block". Every not-ready state says what's missing, **when** (date, count, or "timing unknown because …"), what you can do (or "nothing you need to do"), plus the panel footnote. | **Yes** where "when" or the footnote is missing. |
